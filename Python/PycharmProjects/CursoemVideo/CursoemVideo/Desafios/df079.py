...
while True:
    ...
    if num not in n:
        ...
        print('Número duplicado! Não vou salvar!')
    continua = str(input('Quer continuar?[S/N] ')).strip()
    while continua not in 'SsNn':
        continua = str(input(f'{cor[1]}RESPOSTA NEGADA, TENTE NOVAMENTE{limpa}. Quer continuar?[S/N] ')).strip()
    if continua in 'Nn':
        break
print(f'A lista de números formada foi {sorted(n)}')
